...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = '無法連線至執行此伺服器的主機，請再試一次。';
        } else {
            $message = sprintf('與執行此伺服器的主機通訊時發生錯誤。此錯誤已被記錄，請再試一次。（代碼：%s）（request_id：%s）', $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf('遠端主機發生錯誤：%s。（request id：%s）', $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
