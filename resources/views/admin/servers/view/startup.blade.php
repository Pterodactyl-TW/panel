...
@section('content-header')
    <h1>{{ $server->name }}<small>控管啟動指令與變數。</small></h1>
    <ol class="breadcrumb">
...
<form action="{{ route('admin.servers.view.startup', $server->id) }}" method="POST">
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">修改啟動指令</h3>
                </div>
                <div class="box-body">
                    <label for="pStartup" class="form-label">啟動指令</label>
                    <input id="pStartup" name="startup" class="form-control" type="text" value="{{ old('startup', $server->startup) }}" />
                    <p class="small text-muted">在此編輯你伺服器的啟動指令。以下變數預設可用：<code>@{{SERVER_MEMORY}}</code>、<code>@{{SERVER_IP}}</code> 及 <code>@{{SERVER_PORT}}</code>。</p>
                </div>
                <div class="box-body">
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">Docker 映像檔組態設定</h3>
                </div>
                <div class="box-body">
                    <div class="form-group">
                        <label for="pDockerImage">映像檔</label>
                        <select id="pDockerImage" name="docker_image" class="form-control"></select>
                        <input id="pDockerImageCustom" name="custom_docker_image" value="{{ old('custom_docker_image') }}" class="form-control" placeholder="或輸入自訂映像檔..." style="margin-top:1rem"/>
                        <p class="small text-muted no-margin">這是用來執行此伺服器的 Docker 映像檔。可從下拉選單選擇一個映像檔，或在上方文字欄位輸入自訂映像檔。</p>
                    </div>
                </div>
            </div>
        </div>
...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: '選擇一個 Nest Egg'}).on('change', function () {
            ...
            if (!_.get(objectChain, 'startup', false)) {
                $('#pDefaultStartupCommand').val(_.get(parentChain, 'startup', '錯誤：未定義啟動指令！'));
            } else {
        ...
        $('#pNestId').select2({placeholder: '選擇一個 Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
