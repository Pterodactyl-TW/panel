...
@section('content-header')
    <h1>{{ $node->name }}<small>控管此節點上伺服器可用的連接埠配置。</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">管理</a></li>
        <li><a href="{{ route('admin.nodes') }}">節點</a></li>
        <li><a href="{{ route('admin.nodes.view', $node->id) }}">{{ $node->name }}</a></li>
        <li class="active">連接埠配置</li>
    </ol>
@endsection
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="{{ route('admin.nodes.view.allocation', $node->id) }}">連接埠配置</a></li>
    ...
    <div class="col-sm-4">
        <form action="{{ route('admin.nodes.view.allocation', $node->id) }}" method="POST">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">指派新連接埠配置</h3>
                </div>
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <p class="text-muted small">在此輸入要指派連接埠的 IP 位址。</p>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <p class="text-muted small">若你想為這些配置指派預設別名，請在此輸入。</p>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <p class="text-muted small">在此輸入個別連接埠或連接埠範圍，以逗號或空白分隔。</p>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">刪除該 IP 區塊的所有連接埠配置</h4>
            </div>
            <form action="{{ route('admin.nodes.view.allocation.removeBlock', $node->id) }}" method="POST">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-danger">刪除連接埠配置</button>
                </div>
            </form>
        </div>
    </div>
    ...
    function deleteSelected() {
        ...
        if (selectedItems.length !== 0) {
            ...
            if (selectedItems.length > 5) {
                formattedItems += '，以及其他 ' + (selectedItems.length - 5) + ' 項';
            }

            swal({
                type: 'warning',
                title: '',
                text: '你確定要刪除以下連接埠配置：' + formattedItems + '？',
                html: true,
                showCancelButton: true,
